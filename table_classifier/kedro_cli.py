...
def run(tag, env, parallel, runner, node_names, to_nodes, from_nodes):
    """Run the pipeline."""
    from table_classifier.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=table_classifier"]
    )
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    """
    from table_classifier.run import ProjectContext
    ...
    kedro_package_name = "table_classifier"
